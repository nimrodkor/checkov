...
class OrConnectionSolver(ComplexConnectionSolver):
    ...
    #             if f[CustomAttributes.ID] not in [p[CustomAttributes.ID] for p in passed_connections] and not failed_by_hash.get(f[CustomAttributes.HASH]):
    ...
    #         if data["count"] == len(connection_queries):
