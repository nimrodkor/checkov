...
from checkov.graph.checks.checks_infra.registry import Registry
...
from checkov.graph.graph_builder.graph_to_tf_definitions import convert_graph_vertices_to_tf_definitions
...
class Runner(BaseRunner):
    ...
    def get_graph_checks_report(self, root_folder, breadcrumbs):
        ...
        for check_id, check_results in checks_results.items():
            for check_result in check_results:
                ...
                if entity_context:
                    full_file_path = entity[CustomAttributes.FILE_PATH.value]
                    copy_of_check_result = copy.deepcopy(check_result)
                    copy_of_check_result['entity'] = entity.get(CustomAttributes.CONFIG.value)
    ...
    def get_entity_context_and_evaluations(self, entity):
        ...
        block_type = entity[CustomAttributes.BLOCK_TYPE.value]
        full_file_path = entity[CustomAttributes.FILE_PATH.value]
        definition_path = entity[CustomAttributes.BLOCK_NAME.value].split('.')
        entity_context = None
