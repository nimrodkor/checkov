...
from checkov.graph.graph_builder.graph_components.attribute_names import CustomAttributes, reserved_attribute_names, \
    EncryptionValues
...
from checkov.graph.graph_builder.graph_components.generic_resource_encryption import ENCRYPTION_BY_RESOURCE_TYPE
from checkov.graph.graph_builder.utils import is_local_path
from checkov.graph.utils.utils import get_referenced_vertices_in_value, update_dictionary_attribute, \
    join_trimmed_strings, \
    filter_sub_keys
from checkov.graph.utils.utils import remove_index_pattern_from_str, calculate_hash
from checkov.graph.variable_rendering.renderer import VariableRenderer


class LocalGraph:
    def __init__(self, module, module_dependency_map, state_file_map=None):
        self.module = module
        self.vertices = []
        self.edges = []
        ...
        self.state_file_map = state_file_map
        self.module_dependency_map = module_dependency_map
        self.map_path_to_module = {}
        self.is_runtime = state_file_map is not None
    ...
    def _get_aliases(self):
        ...
        for vertex in self.vertices:
            if 'alias' in vertex.attributes:
                aliases[vertex.name] = {CustomAttributes.BLOCK_TYPE.value: vertex.block_type}
        return aliases
    ...
    def update_vertices_breadcrumbs_and_module_connections(self):
        ...
        for vertex in self.vertices:
            ...
            if vertex.source_module > -1:
                ...
                vertex.breadcrumbs[CustomAttributes.SOURCE_MODULE.value] = source_module_data
    ...
    def calculate_encryption_attribute(self):
        for vertex_index in self.vertices_by_block_type.get(BlockType.RESOURCE.value, []):
            ...
            if encryption_conf:
                ...
                vertex.attributes[CustomAttributes.ENCRYPTION.value] = \
                    EncryptionValues.ENCRYPTED.value if is_encrypted else EncryptionValues.UNENCRYPTED.value
                vertex.attributes[CustomAttributes.ENCRYPTION_DETAILS.value] = reason
