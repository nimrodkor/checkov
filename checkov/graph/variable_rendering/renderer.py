...
from checkov.graph.utils.utils import get_referenced_vertices_in_value, run_function_multithreaded, calculate_hash, \
    join_trimmed_strings, remove_interpolation, remove_index_pattern_from_str, remove_function_calls_from_str
from checkov.graph.variable_rendering.evaluate_terraform import replace_string_value, evaluate_terraform
...
class VariableRenderer:
    ...
    def _edge_evaluation_task(self, edges):
        ...
        if self.local_graph.is_runtime:
            for edge in edges:
    ...
    def extract_value_from_vertex(key_path, attributes):
        ...
        if attributes.get(CustomAttributes.BLOCK_TYPE.value) == BlockType.VARIABLE.value:
            return attributes.get('default')
        if attributes.get(CustomAttributes.BLOCK_TYPE.value) == BlockType.OUTPUT.value:
    ...
    def find_path_from_referenced_vertices(referenced_vertices, vertex_attributes):
        ...
        for vertex_reference in referenced_vertices:
            ...
            if vertex_attributes[CustomAttributes.BLOCK_TYPE.value] == block_type:
                for i in range(len(copy_of_attribute_path)):
                    ...
                    if vertex_attributes[CustomAttributes.BLOCK_NAME.value] == name:
