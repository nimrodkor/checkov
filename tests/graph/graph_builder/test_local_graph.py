...
from checkov.graph.graph_builder.graph_components.attribute_names import CustomAttributes, EncryptionValues, \
    EncryptionTypes
from checkov.graph.graph_builder.graph_components.block_types import BlockType
from checkov.graph.graph_builder.graph_components.blocks import Block
...
from checkov.graph.graph_builder.local_graph import LocalGraph
from checkov.graph.graph_manager import GraphManager
from checkov.graph.utils.utils import calculate_hash, decode_graph_property_value
...
class TestLocalGraph(TestCase):
    ...
    def test_set_variables_values_from_modules(self):
        resources_dir = os.path.realpath(os.path.join(TEST_DIRNAME, '../resources/variable_rendering/render_from_module_vpc'))
    ...
    def test_encryption_aws(self):
        ...
        for attribute_dict in all_attributes:
            [resource_type, resource_name] = decode_graph_property_value(
                attribute_dict[CustomAttributes.ID.value]).split(".")
            if resource_type in ENCRYPTION_BY_RESOURCE_TYPE:
                is_encrypted = json.loads(attribute_dict[CustomAttributes.ENCRYPTION.value])
                details = json.loads(attribute_dict[CustomAttributes.ENCRYPTION_DETAILS.value])
                ...
                self.assertIsNone(attribute_dict.get(CustomAttributes.ENCRYPTION.value))
                self.assertIsNone(attribute_dict.get(CustomAttributes.ENCRYPTION_DETAILS.value))
